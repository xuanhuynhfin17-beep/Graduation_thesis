"""
05h_compare_ppo_td3_adaptive_no_trade_band_v3c.py

Adaptive no-trade band experiment for risk-aware residual-delta deep hedging.

Main purpose:
    Compare PPO vs TD3 under residual-delta V3C with a learned state-dependent no-trade band.
...
Environment:
    ...
    - Residual-delta action:
          h_t^des = clip(delta_t + adjustment_limit * action_res_t, 0, 1)
    - Adaptive no-trade action:
          band_t = band_min + (action_band_t + 1) / 2 * (band_max - band_min)
    - Adaptive no-trade band learned by the policy.
    - Risk-aware reward:
          scaled_pnl
...
Output:
    outputs/comparison_ppo_td3_adaptive_no_trade_band_v3c_100k_3seeds.xlsx
    outputs/comparison_ppo_td3_adaptive_no_trade_band_v3c_100k_3seeds_step_results.parquet
"""
...
# Adaptive no-trade band range. The policy chooses band_t in [BAND_MIN, BAND_MAX].
# Fixed V3C used NO_TRADE_BAND = 0.02. Here that threshold is learned state-by-state.
BAND_MIN = 0.00
BAND_MAX = 0.08
BAND_WIDTH_PENALTY = 0.00
...
MODEL_DIR = OUTPUT_DIR / "ppo_td3_adaptive_no_trade_band_v3c_models"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_OUTPUT_PATH = OUTPUT_DIR / "comparison_ppo_td3_adaptive_no_trade_band_v3c_100k_3seeds.xlsx"
STEP_RESULTS_PATH = OUTPUT_DIR / "comparison_ppo_td3_adaptive_no_trade_band_v3c_100k_3seeds_step_results.parquet"
...
class OptionHedgingEnvResidualDelta(gym.Env):
    ...
    def __init__(
        ...
        band_width_penalty: float = BAND_WIDTH_PENALTY,
        ...
        self.band_width_penalty = float(band_width_penalty)
        if self.band_max < self.band_min:
            raise ValueError("band_max must be >= band_min")
        ...
        #   action[0] = residual hedge action in [-1, 1]
        #   action[1] = no-trade band action in [-1, 1]
    ...
    def _scale_action_to_hedge(
        self,
        action: np.ndarray,
        row: pd.Series,
    ) -> tuple[float, float, float, float, float, float]:
        # action[0]: residual hedge adjustment around delta
        # action[1]: adaptive no-trade band width
        raw_residual_action = float(np.clip(action[0], -1.0, 1.0))
        raw_band_action = float(np.clip(action[1], -1.0, 1.0))

        delta = self._get_delta(row)

        desired_hedge = float(
            np.clip(
                delta + self.adjustment_limit * raw_residual_action,
                self.hedge_min,
                self.hedge_max,
            )
        )
        ...
        adaptive_band = float(
            self.band_min
            + (raw_band_action + 1.0) / 2.0 * (self.band_max - self.band_min)
        )

        if abs(desired_hedge - self.prev_hedge) < adaptive_band:
    ...
    def step(self, action):
        ...
        raw_residual_action, raw_band_action, adaptive_band, desired_hedge, target_hedge, adjustment = self._scale_action_to_hedge(action, row)
        delta = self._get_delta(row)
        ...
        # Optional penalty to discourage the policy from always selecting the widest band.
        # Default is 0.0, so the band is learned purely through PnL/risk/cost trade-offs.
        if self.band_max > self.band_min:
            normalized_band = (adaptive_band - self.band_min) / (self.band_max - self.band_min)
        else:
            normalized_band = 0.0
        ...
        training_reward = (
            ...
            - self.band_width_penalty * (normalized_band ** 2)
        )
        ...
        info = {
            ...
            "raw_action": raw_residual_action,
            "raw_residual_action": raw_residual_action,
            "raw_band_action": raw_band_action,
    ...
    for episode_id, ep in df.groupby("EPISODE_ID"):
        ...
        for t in range(len(ep)):
            ...
            desired_hedge = float(np.clip(
                delta + ADJUSTMENT_LIMIT * raw_residual_action,
                HEDGE_MIN,
                HEDGE_MAX,
            ))

            adaptive_band = float(
                BAND_MIN + (raw_band_action + 1.0) / 2.0 * (BAND_MAX - BAND_MIN)
            )

            if abs(desired_hedge - prev_hedge) < adaptive_band:
            ...
            deltas.append(delta)
            raw_actions.append(raw_residual_action)
            raw_band_actions.append(raw_band_action)
            adaptive_bands.append(adaptive_band)
        ...
        results.append({
            ...
            "AVG_RAW_BAND_ACTION": np.mean(raw_band_actions),
            "STD_RAW_BAND_ACTION": np.std(raw_band_actions),
            "AVG_ADAPTIVE_BAND": np.mean(adaptive_bands),
            "STD_ADAPTIVE_BAND": np.std(adaptive_bands),
            "MIN_ADAPTIVE_BAND": np.min(adaptive_bands),
            "MAX_ADAPTIVE_BAND": np.max(adaptive_bands),
            ...
            "BAND_ACTION_NEAR_NEG1_RATE": np.mean(np.array(raw_band_actions) < -0.95),
            "BAND_ACTION_NEAR_POS1_RATE": np.mean(np.array(raw_band_actions) > 0.95),
            "STRATEGY": strategy_name,
...
def make_env(seed: int) -> Monitor:
    env = OptionHedgingEnvResidualDelta(
        ...
        band_min=BAND_MIN,
        band_max=BAND_MAX,
        band_width_penalty=BAND_WIDTH_PENALTY,
        hedge_min=HEDGE_MIN,
        hedge_max=HEDGE_MAX,
...
def make_model(algorithm: str, env: Monitor, seed: int):
    ...
    if algorithm == "PPO":
        return PPO(
            ...
            tensorboard_log=str(OUTPUT_DIR / "tb_logs" / "adaptive_no_trade_band_v3c"),
            seed=seed,
        )
...
for algorithm in ALGORITHMS:
    algo_dir = MODEL_DIR / algorithm.lower()
    algo_dir.mkdir(parents=True, exist_ok=True)

    for seed in SEEDS:
        print("\n" + "=" * 90)
        print(f"Training adaptive-band {algorithm} residual-delta V3C, seed={seed}")
        print("=" * 90)
        ...
        model_path = algo_dir / f"{algorithm.lower()}_residual_delta_v3c_seed_{seed}"
        ...
        strategy_name = f"{algorithm.lower()}_residual_delta_v3c_seed_{seed}"
        ...
        print(f"\n{algorithm} results by split, seed={seed}:")
        print(seed_metrics)
...
if len(rl_step_results) <= max_excel_rows:
    ...
    step_results_for_excel = rl_step_results.sample(
        n=max_excel_rows,
        random_state=42,
    ).sort_values(["ALGORITHM", "SEED", "EPISODE_ID", "QUOTE_DATE"])
...
adaptive_band_config = pd.DataFrame([{
    "ADJUSTMENT_LIMIT": ADJUSTMENT_LIMIT,
    "BAND_MIN": BAND_MIN,
    "BAND_MAX": BAND_MAX,
    "BAND_WIDTH_PENALTY": BAND_WIDTH_PENALTY,
...
with pd.ExcelWriter(EXCEL_OUTPUT_PATH, engine="openpyxl") as writer:
    adaptive_band_config.to_excel(writer, sheet_name="Adaptive_Band_Config", index=False)
