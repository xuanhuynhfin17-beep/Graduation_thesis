...
DELTA_RISK_PENALTY = 0.30
EXTRA_COST_PENALTY = 0.00  # cost is already included in raw PnL; keep 0 first

TOTAL_TIMESTEPS = 100_000
SEEDS = [1,2,3,4,5]

PPO_MODEL_DIR = OUTPUT_DIR / "ppo_residual_delta_v3b_models"
PPO_MODEL_DIR.mkdir(parents=True, exist_ok=True)

PPO_COMPARISON_PATH = OUTPUT_DIR / "comparison_baseline_ppo_residual_delta_v3b.xlsx"
    ...
    for episode_id, ep in df.groupby("EPISODE_ID"):
        ...
        for t in range(len(ep)):
            ...
            step_rows.append({
                ...
                "STRATEGY_FAMILY": "ppo_residual_delta_v3b_multiseeds",
                "RAW_ACTION": raw_action,
                "DELTA": delta,
                "DESIRED_HEDGE": desired_hedge,
                "HEDGE": target_hedge,
...
for seed in SEEDS:
    print("\n" + "=" * 80)
    print(f"Training PPO residual-delta v3b_multiseeds, seed={seed}")
    print("=" * 80)
    ...
    model = PPO(
        ...
        tensorboard_log=str(OUTPUT_DIR / "tb_logs" / "ppo_residual_delta_v3b_multiseeds"),
        seed=seed,
    )
    ...
    model_path = PPO_MODEL_DIR / f"ppo_residual_delta_v3b_multiseeds_seed_{seed}"
    ...
    strategy_name = f"ppo_residual_delta_v3b_multiseeds_seed_{seed}"
    ...
    print("\nPPO residual-delta v3b_multiseeds results by split:")
    print(seed_metrics)
