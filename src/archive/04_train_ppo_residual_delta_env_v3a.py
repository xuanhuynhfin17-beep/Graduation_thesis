...
DELTA_RISK_PENALTY = 0.10
EXTRA_COST_PENALTY = 0.00  # cost is already included in raw PnL; keep 0 first

TOTAL_TIMESTEPS = 100_000
SEEDS = [42]

PPO_MODEL_DIR = OUTPUT_DIR / "ppo_residual_delta_v3a_models"
PPO_MODEL_DIR.mkdir(parents=True, exist_ok=True)

PPO_COMPARISON_PATH = OUTPUT_DIR / "comparison_baseline_ppo_residual_delta_v3a.xlsx"
...
for seed in SEEDS:
    ...
    model = PPO(
        ...
        tensorboard_log=str(OUTPUT_DIR / "tb_logs" / "ppo_residual_delta_v3a"),
        seed=seed,
    )
