...
MODEL_DIR = OUTPUT_DIR / "epsilon_sensitivity_v3c_models"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_OUTPUT_PATH = OUTPUT_DIR / "epsilon_sensitivity_v3c_100k_5seeds.xlsx"
STEP_RESULTS_PATH = OUTPUT_DIR / "epsilon_sensitivity_v3c_100k_5seeds_step_results.parquet"
...
# Epsilon sensitivity scenarios.
EPSILON_GRID = [0.05, 0.10, 0.15, 0.20]

# Default is intentionally modest: PPO only, 4 epsilons x 5 seeds = 20 trainings.
SELECTED_ALGORITHMS = ["PPO"]

BASE_ALGORITHM_CONFIGS = {
    "PPO": {
        "learning_rate": 3e-4,
        "n_steps": 2048,
        "batch_size": 256,
...
def build_epsilon_scenarios() -> list[dict]:
    scenarios = []
    for algo in SELECTED_ALGORITHMS:
        if algo not in BASE_ALGORITHM_CONFIGS:
            raise ValueError(
                f"Unknown algorithm in SELECTED_ALGORITHMS: {algo}. "
                "This fixed script currently supports PPO. Use the broader hyperparameter package for SAC/TD3."
            )
        for eps in EPSILON_GRID:
            config = dict(BASE_ALGORITHM_CONFIGS[algo])
            eps_label = f"{eps:.2f}".replace(".", "p")
            config.update({
                "VARIANT": f"{algo}_eps_{eps_label}",
                "ALGORITHM": algo,
                "EPSILON": eps,
                "DESCRIPTION": f"{algo} V3C residual-delta sensitivity with epsilon={eps:.2f}.",
                "RESIDUAL_DELTA": True,
                "ADJUSTMENT_LIMIT": eps,
...
HYPERPARAM_SCENARIOS = build_epsilon_scenarios()
...
print(FEATURE_COLS)
print("\nEpsilon sensitivity scenarios:")
print(pd.DataFrame(HYPERPARAM_SCENARIOS)[["VARIANT", "ALGORITHM", "EPSILON", "DESCRIPTION"]])
...
def make_model(env: Monitor, seed: int, scenario: dict | None = None) -> PPO:
    ...
    return PPO(
        ...
        tensorboard_log=str(OUTPUT_DIR / "tb_logs" / "epsilon_sensitivity_v3c"),
        seed=seed,
    )
    ...
    for episode_id, ep in df.groupby("EPISODE_ID"):
        ...
        for t in range(len(ep)):
            ...
            step_rows.append({
                "VARIANT": variant_name,
                "ALGORITHM": algo,
                "EPSILON": variant.get("EPSILON", np.nan),
                "SEED": seed,
                "EPISODE_ID": episode_id,
                "SPLIT": split,
                "QUOTE_DATE": row["QUOTE_DATE"],
...
def make_scenario_summary(metrics_by_seed: pd.DataFrame) -> pd.DataFrame:
    ...
    summary = (
        rl.groupby(["ALGORITHM", "VARIANT", "EPSILON", "SPLIT"], dropna=False)
        .agg(
            N_SEEDS=("SEED", "nunique"),
            MEAN_OF_MEAN_PNL=("MEAN_PNL", "mean"),
            STD_OF_MEAN_PNL=("MEAN_PNL", "std"),
    ...
    return summary.sort_values(["SPLIT", "ALGORITHM", "EPSILON", "VARIANT"]).reset_index(drop=True)
...
# For epsilon sensitivity, all scenarios are already built from EPSILON_GRID and SELECTED_ALGORITHMS.
selected_scenarios = list(HYPERPARAM_SCENARIOS)

print("\nSelected epsilon-sensitivity scenarios:")
...
for scenario in selected_scenarios:
    ...
    for seed in SEEDS:
        print("\n" + "=" * 90)
        print(f"Training {algo} scenario={variant_name}, seed={seed}")
        print("=" * 90)

        train_env = make_env(seed=seed, variant=scenario)
        model = make_model(env=train_env, seed=seed, scenario=scenario)
        ...
        model_path = variant_dir / f"{algo.lower()}_{variant_name}_seed_{seed}"
        ...
        seed_metrics = make_metrics(ep_df, ["VARIANT", "ALGORITHM", "EPSILON", "SEED", "SPLIT", "STRATEGY"])

        print(f"\nResults by split, scenario={variant_name}, seed={seed}:")
        print(seed_metrics)
        ...
        training_log_rows.append({
            "VARIANT": variant_name,
            "ALGORITHM": algo,
            "EPSILON": scenario.get("EPSILON", np.nan),
            "SEED": seed,
...
comparison_metrics = make_metrics(all_episode_results, ["VARIANT", "ALGORITHM", "EPSILON", "SPLIT", "STRATEGY"])
