...
PPO_MODEL_DIR = OUTPUT_DIR / "ppo_residual_delta_v3c_highcost_multiseed_models"
PPO_MODEL_DIR.mkdir(parents=True, exist_ok=True)

PPO_COMPARISON_PATH = OUTPUT_DIR / "comparison_baseline_ppo_residual_delta_v3c_highcost_multiseed.xlsx"
    ...
    for episode_id, ep in df.groupby("EPISODE_ID"):
        ...
        for t in range(len(ep)):
            ...
            step_rows.append({
                ...
                "STRATEGY_FAMILY": "ppo_residual_delta_v3c_highcost",
                "RAW_ACTION": raw_action,
                "DELTA": delta,
                "DESIRED_HEDGE": desired_hedge,
                "HEDGE": target_hedge,
...
for seed in SEEDS:
    print("\n" + "=" * 80)
    print(f"Training PPO residual-delta v3c high-cost, seed={seed}")
    print("=" * 80)
    ...
    model = PPO(
        ...
        tensorboard_log=str(OUTPUT_DIR / "tb_logs" / "ppo_residual_delta_v3c_highcost_multiseed"),
        seed=seed,
    )
    ...
    model_path = PPO_MODEL_DIR / f"ppo_residual_delta_v3c_highcost_seed_{seed}"
    ...
    strategy_name = f"ppo_residual_delta_v3c_highcost_seed_{seed}"
...
def run_baseline_for_current_cost(transitions_df, strategy="delta"):
    df = transitions_df.copy()
    df = df.sort_values(["EPISODE_ID", "QUOTE_DATE"]).reset_index(drop=True)
    ...
    episode_result["STRATEGY_FAMILY"] = strategy

    return episode_result


def make_highcost_baseline_results(transitions_df):
...
ppo_ep_all["COST_SCENARIO"] = "high_transaction_cost_0.001"
ppo_steps_all["COST_SCENARIO"] = "high_transaction_cost_0.001"
# ============================================================
# COMPARE BASELINE VS PPO V2
# ============================================================

baseline_results_for_export = make_highcost_baseline_results(transitions)
baseline_results_for_export["SEED"] = np.nan
baseline_results_for_export["COST_SCENARIO"] = "high_transaction_cost_0.001"
