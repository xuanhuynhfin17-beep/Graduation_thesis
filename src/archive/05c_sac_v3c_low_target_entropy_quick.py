"""
05c_sac_v3c_low_target_entropy_quick.py

SAC-C quick diagnostic for risk-aware residual-delta deep hedging.

This run uses SAC only, automatic entropy tuning with target_entropy=-0.25, SEEDS=[1], TOTAL_TIMESTEPS=50_000.
...
Output:
    outputs/comparison_sac_v3c_low_target_entropy_quick.xlsx
    outputs/comparison_sac_v3c_low_target_entropy_quick_step_results.parquet
"""
...
MODEL_DIR = OUTPUT_DIR / "sac_v3c_low_target_entropy_quick_models"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_OUTPUT_PATH = OUTPUT_DIR / "comparison_sac_v3c_low_target_entropy_quick.xlsx"
STEP_RESULTS_PATH = OUTPUT_DIR / "comparison_sac_v3c_low_target_entropy_quick_step_results.parquet"
...
def make_model(algorithm: str, env: Monitor, seed: int):
    ...
    if algorithm == "PPO":
        return PPO(
            ...
            tensorboard_log=str(OUTPUT_DIR / "tb_logs" / "sac_v3c_low_target_entropy_quick"),
            seed=seed,
        )
...
for algorithm in ALGORITHMS:
    ...
    for seed in SEEDS:
        print("\n" + "=" * 90)
        print(f"Training SAC-C low-target-entropy residual-delta V3C, seed={seed}")
        print("=" * 90)
