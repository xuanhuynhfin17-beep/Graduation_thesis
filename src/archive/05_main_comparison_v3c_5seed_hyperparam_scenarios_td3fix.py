...
MODEL_DIR = OUTPUT_DIR / "main_comparison_v3c_hyperparam_scenarios_td3_models"
MODEL_DIR.mkdir(parents=True, exist_ok=True)

EXCEL_OUTPUT_PATH = OUTPUT_DIR / "main_comparison_v3c_hyperparam_scenarios_100k_5seeds_TD3_only.xlsx"
STEP_RESULTS_PATH = OUTPUT_DIR / "main_comparison_v3c_hyperparam_scenarios_100k_5seeds_TD3_only_step_results.parquet"
...
class TD3PolicyNoSDE(TD3Policy):
    """Compatibility wrapper for SB3 versions that pass use_sde into TD3Policy.

    Some stable-baselines3 installations pass gSDE-related keyword arguments
    through the off-policy algorithm base class. TD3Policy does not use gSDE,
    so this wrapper drops those arguments before calling the standard TD3Policy.
    """

    def __init__(self, *args, use_sde=False, **kwargs):
        kwargs.pop("sde_sample_freq", None)
        kwargs.pop("use_sde_at_warmup", None)
        kwargs.pop("log_std_init", None)
        kwargs.pop("full_std", None)
        kwargs.pop("use_expln", None)
        kwargs.pop("squash_output", None)
...
def make_model(env: Monitor, seed: int, scenario: dict):
    ...
    tb_dir = OUTPUT_DIR / "tb_logs" / "main_comparison_v3c_hyperparam_scenarios_td3_models"
