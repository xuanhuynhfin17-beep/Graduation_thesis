"""
10d_pretrain_finetune_quantile_option_state.py

PPO pretraining extension using:
    HMM-MSGBM + Gradient Boosting Quantile option-state simulator.

Experiment:
    E4_ms_gbm_quantile_option_state_pretrain

Outputs:
    outputs/pretraining_quantile_option_state_results.xlsx
    outputs/pretraining_quantile_option_state_step_results.parquet

Run pilot:
    py src\10d_pretrain_finetune_quantile_option_state.py --mode pilot --pretrain_steps 50000 --finetune_steps 50000 --seeds 1

Run final:
    py src\10d_pretrain_finetune_quantile_option_state.py --mode final --pretrain_steps 150000 --finetune_steps 150000 --seeds 1 2 3
"""
...
MODEL_DIR = OUT_DIR / "pretraining_quantile_option_state_models"
OUT_DIR.mkdir(exist_ok=True)
MODEL_DIR.mkdir(exist_ok=True)

REAL_FILE = DATA_DIR / "transitions_daily_top1_final_with_spy_2010_2023.parquet"
SIM_DEFAULT = SIM_DIR / "ms_gbm_quantile_option_state_final_n5000.parquet"
...
def add_engineered_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for c in FEATURE_COLS + ["SPY_DS", "DOPTION", "SPY_CLOSE", "SPY_NEXT_CLOSE", "OPTION_MID"]:
        if c in out.columns:
            out[c] = pd.to_numeric(out[c], errors="coerce").replace([np.inf, -np.inf], np.nan)
    out = out.dropna(subset=[c for c in FEATURE_COLS if c in out.columns] + ["SPY_DS", "DOPTION", "SPY_CLOSE", "SPY_NEXT_CLOSE", "OPTION_MID"])
    return out
...
class OptionHedgingEnvResidualDelta(gym.Env):
    ...
    def _obs(self):
        ...
        delta = self._delta(row)
        return np.concatenate([x, np.array([self.prev_hedge, self.prev_hedge - delta], dtype=np.float32)]).astype(np.float32)
    ...
    def step(self, action):
        ...
        final_tc = 0.0
        if is_last:
            final_tc = self._tc(row, hedge, "SPY_NEXT_CLOSE")
            tc += final_tc
        ...
        next_dte = float(row.get("NEXT_DTE", max(dte - 1, 0)))
        dt = max(dte - next_dte, 1.0) / 252.0
        delta_risk = (((hedge - delta) * iv * float(row["SPY_CLOSE"])) ** 2) * dt / (scale ** 2)
        reward = scaled - DOWNSIDE_PENALTY * downside ** 2 - DELTA_RISK_PENALTY * delta_risk - EXTRA_COST_PENALTY * (tc / scale)
...
def evaluate_model(model, real_df, seed, feature_cols, feature_mean, feature_std, training_time_min):
    step_rows, ep_rows = [], []
    ...
    for episode_id, ep in df.groupby("EPISODE_ID"):
        ...
        prev_hedge = 0.0
        rewards, tcs, turnovers, hedges, deltas, actions, adjustments, no_trades = [], [], [], [], [], [], [], []
        ...
        for t in range(len(ep)):
            ...
            tc = compute_trade_cost(row, trade_size, False)
            is_last = t == len(ep) - 1
            turnover = abs(trade_size)
            if is_last:
                tc += compute_trade_cost(row, hedge, True)
                turnover += abs(hedge)
            pnl = -float(row["DOPTION"]) + hedge * float(row["SPY_DS"]) - tc

            rewards.append(pnl); tcs.append(tc); turnovers.append(turnover); hedges.append(hedge); deltas.append(delta)
            actions.append(raw_action); adjustments.append(adjustment); no_trades.append(abs(trade_size) < 1e-12)
            step_rows.append({
                "EXPERIMENT": "E4_ms_gbm_quantile_option_state_pretrain",
                "SEED": seed,
        ...
        ep_rows.append({
            ...
            "STRATEGY": f"ppo_quantile_pretrain_seed_{seed}",
...
def make_model(env, seed):
    return PPO(
        ...
        seed=seed,
        tensorboard_log=str(OUT_DIR / "tb_logs" / "quantile_option_state_pretrain"),
    )
...
def run(args):
    ...
    for seed in args.seeds:
        print("=" * 100)
        print(f"E4 quantile option-state pretraining seed={seed}")
        print("=" * 100)
        start = time.time()
        ...
        model_path = MODEL_DIR / f"E4_quantile_option_state_seed_{seed}"
        model.save(model_path)

        steps, eps = evaluate_model(model, real, seed, feature_cols, feature_mean, feature_std, elapsed)
        all_steps.append(steps); all_eps.append(eps)
        logs.append({
            "EXPERIMENT": "E4_ms_gbm_quantile_option_state_pretrain",
            "SEED": seed,
    ...
    summary = (
        metrics_by_seed.groupby(["EXPERIMENT", "SPLIT"])
        .agg(
    ...
    suffix = "pilot" if args.mode == "pilot" else "final"
    xlsx = OUT_DIR / f"pretraining_quantile_option_state_results_{suffix}.xlsx"
    parquet = OUT_DIR / f"pretraining_quantile_option_state_step_results_{suffix}.parquet"
    ...
    with pd.ExcelWriter(xlsx, engine="openpyxl") as writer:
        ...
        summary.to_excel(writer, sheet_name="Experiment_Summary", index=False)
...
def main():
    ...
    parser.add_argument("--sim", type=str, default=str(SIM_DEFAULT))
    parser.add_argument("--pretrain_steps", type=int, default=50000)
